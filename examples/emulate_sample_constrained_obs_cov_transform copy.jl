...
function emulate(eki, inverse_problem, calibration_outputs, noise_covariance;
    ...
    emulator_training_data, gauss_process = emulate(X, Ĝ; validation_indices, kernel, α = 1e-3)
    ...
    unscaled_chain_X_emulated = inverse_normalize_transform(hcat(chain_X_emulated...), normalization_transformation)
    # unscaled_chain_X_emulated = [samples_emulated[:,j] for j in axes(samples_emulated)[2]]
    ...
    unscaled_chain_X = inverse_normalize_transform(hcat(chain_X...), normalization_transformation)
    ...
    begin
        ...
        emulator_best = unscaled_chain_X_emulated[:, argmax(chain_nll_emulated)]
        true_best = unscaled_chain_X[:, argmax(chain_nll_emulated)]
        ...
        emulator_mean = mean(unscaled_chain_X_emulated, dims=2)
        true_mean = mean(unscaled_chain_X, dims=2)
